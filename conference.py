...
# - - - Profile objects - - - - - - - - - - - - - - - - - - -
    ...
    def _getProfileFromUser(self):
        ...
        if not user:
            raise endpoints.UnauthorizedException('Authorization required -- Please login with your Google+ account')
    ...
    def _doProfile(self, save_request=None):
        ...
        if save_request:
            ...
            # put modified profile to datastore
            prof.put()
...
# - - - Conference objects - - - - - - - - - - - - - - - - - - -
    ...
    def filterPlayground(self, request):
        ...
        q = Conference.query().filter(Conference.city == "Paris").filter(Conference.maxAttendees > 40).order(-Conference.maxAttendees).order(Conference.name)
        # simple filter usage:
        # q = q.filter(Conference.city == "Paris")


        """q = q.filter(Conference.maxAttendees > 40)
